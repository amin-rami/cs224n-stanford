...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        c_0 = torch.flatten(torch.permute(dec_init_state[1], (1, 0, 2)), 1)
        h_0 = self.h_projection(h_0)
        c_0 = self.c_projection(c_0)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings(target_padded, 'target')
        for t in range(Y.size(0)):
            Y_t = Y[t]
            Ybar_t = torch.concatenate((Y_t, o_prev), dim=-1)
